...
def get_section_opics_project_code(optics_branch, proj, project_branch,  pull_time_root_name):
    ...
    s += f'    if [ -f /{proj_pull_dir}/opics_inter/inter/planner/pddl/pddlstream_local/.gitmodules ]; then \n'
    s += f'        echo " - "\n'
    ...
    s += f'        git submodule update --init --recursive\n'
    s += f'        if [ -d /{proj_pull_dir}/opics_inter/inter/planner/pddl/pddlstream_local/downward ]; then \n'
    s += f'            echo " - "\n'
