...
def generate_run_script(proj, pull_time_root_name, optics_spec_fname, spec_name):
    ...
    s += f'        if [ -z "$2" ]; then\n'
    s += f'            echo "optics for inter requires scene type handling as additional arg: scene_type_provided|scene_type_deduced"\n'
    s += f'            exit 1\n'
    s += f'        else\n'
    ...
    s += f'            else\n'
    s += f'                echo "scene_type_handling must be scene_type_provided|scene_type_deduced"\n'
    s += f'                exit 1\n'
